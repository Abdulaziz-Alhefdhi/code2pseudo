...
from keras.layers import Input, SimpleRNN, Dense, Embedding, Activation, dot, concatenate, TimeDistributed
...
def build_model(latent_dim, num_encoder_tokens, num_decoder_tokens):
    ...
    _, state = SimpleRNN(latent_dim, return_state=True)(en_x)

    decoder_inputs = Input(shape=(None,))
    de_x = Embedding(num_decoder_tokens+1, latent_dim, mask_zero=True)(decoder_inputs)
    decoder_outputs = SimpleRNN(latent_dim, return_sequences=True)(de_x, initial_state=state)
    model_outputs = Dense(num_decoder_tokens, activation="softmax")(decoder_outputs)


    return encoder_inputs, decoder_inputs, model_outputs
